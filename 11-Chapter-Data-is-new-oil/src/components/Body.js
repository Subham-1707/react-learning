...
const Body = () => {
  ...
  const {user, setUser } = useContext(UserContext) // get the vaule of user and setUser from UserContext. user is an object with name and email properties. setUser is a function to update the user object.
  ...
  return allRestaurants?.length === 0 ? <Shimmer /> : (
    <div className="w-[90%] flex flex-col items-center transition-all">
      ...
      <input  className='text-l border-2 p-2' type='text' 
       placeholder='Enter Name'
       value={user.name} 
        onChange = {(e) => setUser({...user, name: e.target.value})}
      />
      <input  className='text-l m-1 border-2 p-2' type='text' 
